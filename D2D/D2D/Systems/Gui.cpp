...
void Gui::Render()
{
	...
	(
		...
		//ImGuiWindowFlags_NoDocking | //Delete
...
void Gui::RenderText(float x, float y, float r, float g, float b, string content)
{
	GuiText text;
	text.Position = Vector2(x, y);
	text.Color = Color(r, g, b, 1);
	text.Content = content;

	RenderText(text);
}

void Gui::RenderText(Vector2 position, Color color, string content)
{
	GuiText text;
...
void Gui::ApplyStyle()
{
	...
	style.Colors[ImGuiCol_PlotHistogram] = highlightBlue; // progress bar
